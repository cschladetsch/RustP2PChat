...
impl E2EEncryption {
    ...
    pub fn get_public_key_base64(&self) -> Result<String> {
        let public_key_der = self.public_key.to_public_key_der()
    ...
    pub fn set_peer_public_key(&mut self, key_base64: &str) -> Result<()> {
        ...
        self.peer_public_key = Some(
            RsaPublicKey::from_public_key_der(&key_bytes)
                .map_err(|e| ChatError::Encryption(format!("Failed to parse public key: {}", e)))?
        );
    ...
    pub fn generate_shared_key(&mut self) -> Result<String> {
        ...
        let encrypted_key = peer_key.encrypt(&mut OsRng, Pkcs1v15Encrypt, &key_bytes)
    ...
    pub fn set_shared_key(&mut self, encrypted_key_base64: &str) -> Result<()> {
        let encrypted_key = general_purpose::STANDARD.decode(encrypted_key_base64)
            .map_err(|e| ChatError::Encryption(format!("Failed to decode encrypted key: {}", e)))?;
        
        let key_bytes = self.private_key.decrypt(Pkcs1v15Encrypt, &encrypted_key)
