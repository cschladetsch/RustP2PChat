...
use rust_p2p_chat::{Config, P2PChat};
...
async fn test_message_protocol_integration() {
    ...
    let messages = vec![
        ...
        Message::new_text(
            "This is a longer message with multiple words and punctuation.".to_string(),
        ),
...
async fn test_file_transfer_with_directories() {
    ...
    fs::write(&nested_file, "File in deep directory")
        .await
        .unwrap();
...
async fn test_configuration_persistence() {
    ...
    assert_eq!(
        ...
        loaded_config.enable_encryption
    );
    assert_eq!(
        original_config.auto_open_media,
        loaded_config.auto_open_media
    );
...
async fn test_graceful_shutdown_simulation() {
    ...
    fs::write(downloads_path.join("test_download.txt"), "test content")
        .await
        .unwrap();
    ...
    let content = fs::read_to_string(downloads_path.join("test_download.txt"))
        .await
        .unwrap();
