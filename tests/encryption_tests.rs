...
async fn test_e2e_encryption_creation() {
    let encryption = E2EEncryption::new().await;
...
async fn test_e2e_encryption_get_public_key() {
    let encryption = E2EEncryption::new().await.unwrap();
...
async fn test_e2e_encryption_set_peer_public_key_valid() {
    let mut encryption1 = E2EEncryption::new().await.unwrap();
    let encryption2 = E2EEncryption::new().await.unwrap();
    
    // Get public key from encryption2
    let public_key = encryption2.get_public_key().await.unwrap();
    
    // Set it in encryption1
    let result = encryption1.set_peer_public_key(public_key).await;
    assert!(result.is_ok());
}

#[tokio::test]
async fn test_e2e_encryption_set_peer_public_key_invalid() {
    let mut encryption = E2EEncryption::new().await.unwrap();
    
    // Try to set invalid base64
    let result = encryption.set_peer_public_key("invalid_base64!@#".to_string()).await;
    assert!(result.is_err());
    
    // Try to set valid base64 but invalid RSA key
    let invalid_key = general_purpose::STANDARD.encode("not_an_rsa_key");
    let result = encryption.set_peer_public_key(invalid_key).await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_set_peer_public_key_empty() {
    ...
    let result = encryption.set_peer_public_key("".to_string()).await;
    assert!(result.is_err());
...
async fn test_e2e_encryption_generate_shared_key() {
    ...
    let public_key2 = encryption2.get_public_key().await.unwrap();
    encryption1.set_peer_public_key(public_key2).await.unwrap();
    
    // Generate shared key
    let result = encryption1.generate_shared_key().await;
    assert!(result.is_ok());
...
async fn test_e2e_encryption_generate_shared_key_no_peer() {
    ...
    let result = encryption.generate_shared_key().await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_set_shared_key_valid() {
    let mut encryption1 = E2EEncryption::new().await.unwrap();
    let mut encryption2 = E2EEncryption::new().await.unwrap();
    
    // Set up peer relationship
    let public_key1 = encryption1.get_public_key().await.unwrap();
    let public_key2 = encryption2.get_public_key().await.unwrap();
    
    encryption1.set_peer_public_key(public_key2).await.unwrap();
    encryption2.set_peer_public_key(public_key1).await.unwrap();
    
    // Generate shared key on encryption1
    let encrypted_key = encryption1.generate_shared_key().await.unwrap();
    
    // Set it on encryption2
    let result = encryption2.set_shared_key(encrypted_key).await;
    assert!(result.is_ok());
}
...
async fn test_e2e_encryption_set_shared_key_invalid() {
    ...
    // Try to set invalid base64
    let result = encryption.set_shared_key("invalid_base64!@#".to_string()).await;
    assert!(result.is_err());
    ...
    let result = encryption.set_shared_key(invalid_key).await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_set_shared_key_empty() {
    ...
    // Try to set empty key
    let result = encryption.set_shared_key("".to_string()).await;
    assert!(result.is_err());
...
async fn test_e2e_encryption_full_key_exchange() {
    ...
    let encrypted_key = encryption1.generate_shared_key().await.unwrap();
    encryption2.set_shared_key(encrypted_key).await.unwrap();
    
    // Both should now be ready for encryption
    let message = "Test message for encryption";
    let encrypted = encryption1.encrypt_message(message).await;
    assert!(encrypted.is_ok());
    
    let decrypted = encryption2.decrypt_message(&encrypted.unwrap()).await;
...
async fn test_e2e_encryption_encrypt_message_no_key() {
    ...
    let result = encryption.encrypt_message("test").await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_encrypt_empty_message() {
    ...
    // Test empty message
    let result = encryption1.encrypt_message("").await;
    assert!(result.is_ok());
    
    let decrypted = encryption2.decrypt_message(&result.unwrap()).await;
    assert!(decrypted.is_ok());
...
async fn test_e2e_encryption_encrypt_large_message() {
    ...
    let result = encryption1.encrypt_message(&large_message).await;
    assert!(result.is_ok());
...
async fn test_e2e_encryption_encrypt_unicode_message() {
    ...
    let result = encryption1.encrypt_message(unicode_message).await;
    assert!(result.is_ok());
...
async fn test_e2e_encryption_decrypt_invalid_ciphertext() {
    ...
    let result = encryption2.decrypt_message("invalid_base64!@#").await;
    assert!(result.is_err());
    ...
    let result = encryption2.decrypt_message(&invalid_cipher).await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_decrypt_corrupted_ciphertext() {
    ...
    let encrypted = encryption1.encrypt_message(original_message).await.unwrap();
    
    // Corrupt the ciphertext
    let mut corrupted = encrypted.clone();
    corrupted.push('X'); // Corrupt by adding character
    let result = encryption2.decrypt_message(&corrupted).await;
    assert!(result.is_err());
    
    // Try with truncated ciphertext
    let truncated = &encrypted[..encrypted.len()-10];
    let result = encryption2.decrypt_message(truncated).await;
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_decrypt_no_shared_key() {
    ...
    let result = encryption.decrypt_message("dGVzdA==").await; // base64 "test"
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_sign_empty_message() {
    ...
    let signature = encryption.sign_message("").await;
    assert!(signature.is_ok());
...
async fn test_e2e_encryption_verify_signature_valid() {
    let encryption1 = E2EEncryption::new().await.unwrap();
    let mut encryption2 = E2EEncryption::new().await.unwrap();
    ...
    let signature = encryption1.sign_message(message).await.unwrap();
    
    // Verify with encryption2
    let result = encryption2.verify_signature(message, &signature).await;
    assert!(result.is_ok());
...
async fn test_e2e_encryption_verify_signature_invalid() {
    ...
    let result = encryption2.verify_signature("Different message", &signature).await;
    assert!(result.is_ok());
...
async fn test_e2e_encryption_verify_signature_no_peer_key() {
    ...
    let result = encryption.verify_signature("test", "dGVzdA==").await; // base64 "test"
    assert!(result.is_err());
}
...
async fn test_e2e_encryption_verify_signature_invalid_format() {
    ...
    let result = encryption2.verify_signature("test", "invalid_base64!@#").await;
    assert!(result.is_err());
    
    // Try to verify valid base64 but invalid signature
    let invalid_sig = general_purpose::STANDARD.encode("not_a_signature");
    let result = encryption2.verify_signature("test", &invalid_sig).await;
    assert!(result.is_err());
}

#[tokio::test]
async fn test_e2e_encryption_concurrent_operations() {
    let encryption1 = Arc::new(Mutex::new(E2EEncryption::new().await.unwrap()));
    let encryption2 = Arc::new(Mutex::new(E2EEncryption::new().await.unwrap()));
    ...
    let setup_task = tokio::spawn(async move {
        let public_key1 = enc1_clone.lock().await.get_public_key().await.unwrap();
        let public_key2 = enc2_clone.lock().await.get_public_key().await.unwrap();
        
        enc1_clone.lock().await.set_peer_public_key(public_key2).await.unwrap();
        enc2_clone.lock().await.set_peer_public_key(public_key1).await.unwrap();
        
        let encrypted_key = enc1_clone.lock().await.generate_shared_key().await.unwrap();
        enc2_clone.lock().await.set_shared_key(encrypted_key).await.unwrap();
    });
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let encrypted = enc1.lock().await.encrypt_message(&message).await.unwrap();
            let decrypted = enc2.lock().await.decrypt_message(&encrypted).await.unwrap();
...
async fn test_e2e_encryption_key_reuse() {
    ...
    for message in messages {
        let encrypted = encryption1.encrypt_message(message).await.unwrap();
        let decrypted = encryption2.decrypt_message(&encrypted).await.unwrap();
        assert_eq!(decrypted, message);
    }
}
...
async fn test_e2e_encryption_nonce_uniqueness() {
    ...
    for ciphertext in ciphertexts {
        let decrypted = encryption2.decrypt_message(&ciphertext).await.unwrap();
        assert_eq!(decrypted, message);
    }
}
...
fn test_encryption_message_enum() {
    ...
    let public_key_msg = EncryptionMessage::PublicKey("test_key".to_string());
    let shared_key_msg = EncryptionMessage::SharedKey("test_shared".to_string());
    
    match public_key_msg {
        EncryptionMessage::PublicKey(key) => assert_eq!(key, "test_key"),
        _ => panic!("Expected PublicKey variant"),
    }
    
    match shared_key_msg {
        EncryptionMessage::SharedKey(key) => assert_eq!(key, "test_shared"),
...
fn test_message_with_encryption() {
    ...
    let encrypted_msg = Message {
        ...
        message_type: MessageType::EncryptedText("encrypted_content".to_string()),
    };
    
    match encrypted_msg.message_type {
